...
IMAGE_ANALYSIS_SYSTEM_PROMPT = (
    ...
    "Your output will be used as a constrained evidence note for a downstream report-generation model."
)

IMAGE_ANALYSIS_INSTRUCTION = """
[Task]
Review all provided fetal ultrasound images and produce an English evidence note that will be appended to a downstream reasoning prompt.

[Important constraints]
- English only.
- This is NOT the final report and NOT the final diagnosis output.
- Do not rewrite the original task, role, tags, or required output format.
- Stay strictly grounded in the images.
- Preserve uncertainty when the evidence is incomplete or equivocal.
- Extract visible measurements, numbers, and units exactly when possible.
- Do not invent findings, measurements, or conclusions that are not supported by the images.
...
DOWNSTREAM_CONSTRAINT_TEMPLATE = """

[Supplemental image evidence for downstream reasoning]
以下英文内容是由外部视觉分析步骤生成的“图像约束参考”，仅用于辅助后续推理。
请严格遵守：
1. 不要改写上方原始任务、标签、角色设定或输出格式要求。
2. 不要把下面内容当作最终答案直接照抄输出。
3. 只能把其中有图像依据的内容当作补充证据；证据不足时保留不确定性。
4. 不要虚构额外测量值、解剖结构或诊断结论。

[Image constraint reference start]
{completion_text}
[Image constraint reference end]
""".rstrip()
...
def prepare_pure_user_payload(messages: List[Dict[str, Any]], images: List[str]) -> List[Dict[str, Any]]:
    ...
    if isinstance(orig_content, str):
        ...
        if prompt_text:
            prompt_text += "\n\n"
        prompt_text += IMAGE_ANALYSIS_INSTRUCTION
        ...
        for block in content_blocks:
            if block.get("type") == "text":
                ...
                if base_text:
                    base_text += "\n\n"
                block["text"] = base_text + IMAGE_ANALYSIS_INSTRUCTION
                found_text = True
                break
        if not found_text:
            content_blocks.append({"type": "text", "text": IMAGE_ANALYSIS_INSTRUCTION})
    else:
        content_blocks.append({"type": "text", "text": IMAGE_ANALYSIS_INSTRUCTION})
...
def append_constraint_reference_to_first_message(messages: List[Dict[str, Any]], completion_text: str) -> None:
    """
    Append a constrained image-evidence block to the first message without changing the original task.
    """
    ...
    addition = DOWNSTREAM_CONSTRAINT_TEMPLATE.format(completion_text=completion_text.strip())
...
def process_one_record(idx: int, obj: Dict[str, Any], sem: threading.Semaphore) -> Tuple[int, Dict[str, Any]]:
    with sem:
        try:
            ...
            append_constraint_reference_to_first_message(out.get("messages", []), completion_text)
...
def process_single_file(input_path: Path, sem: threading.Semaphore):
    # try:
    #     relative_path = input_path.relative_to(COMMON_BASE)
    # except ValueError:
    #     relative_path = Path(input_path.name)

    # suffix = f".test-{TEST_COUNT}.jsonl" if TEST_COUNT else ".out-gpt5.jsonl"
    # output_path = OUTPUT_ROOT / COMMON_BASE.name / relative_path.parent / input_path.name.replace(".jsonl", suffix)
    ...
    #
    # if not records:
    #     print(f"✅ {input_path.name} 已全部处理完成，跳过。")
    #     return
    #
    # print(f"📦 继续处理 {input_path.name}: 剩余 {len(records)} 条 (已跳过 {processed_count} 条)")

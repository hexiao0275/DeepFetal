...
def get_is_early_from_excel(
    patient_image_root: str,
    excel_path: str,
    col_pid: str = "患者ID",
    col_eid: str = "检查ID",
    col_fid: str = "字段ID",
    col_is_early: str = "是否早孕",
) -> int:
    patient_info = os.path.basename(patient_image_root.rstrip("/"))
    case_re = re.compile(
        r"PatientID(?P<pid>\d+)_ExamID(?P<eid>\d+)(?:_.*?FieldID(?P<fid>\d+))?"
    )
    match = case_re.search(patient_info)
    pid = match.group("pid") if match else ""
    eid = match.group("eid") if match else ""
    fid = match.group("fid") if (match and match.group("fid")) else ""
    print("pid/eid/fid:", pid, eid, fid)
    df = pd.read_excel(excel_path)

    for col in [col_pid, col_eid, col_fid, col_is_early]:
        if col in df.columns:
            df[col] = df[col].astype(str).str.strip()

    if col_is_early not in df.columns:
        return 0

    def any_early(dfx: pd.DataFrame) -> int:
        return 1 if (dfx[col_is_early] == "1").any() else 0

    if pid and eid and fid and all(c in df.columns for c in [col_pid, col_eid, col_fid]):
        hit = df[(df[col_pid] == str(pid)) & (df[col_eid] == str(eid)) & (df[col_fid] == str(fid))]
        if len(hit) > 0:
            return any_early(hit)

    if pid and eid and all(c in df.columns for c in [col_pid, col_eid]):
        hit = df[(df[col_pid] == str(pid)) & (df[col_eid] == str(eid))]
        if len(hit) > 0:
            return any_early(hit)
...
def main(args):
    ...
    if len(os.listdir(tem_early)) != 0:
        print("Running early-pregnancy pipeline...")
        args.image_root = tem_early
        early_process(args)

    if len(os.listdir(tem_normal)) != 0:
        print("Running mid/late-pregnancy pipeline...")
    ...
    if should_use_openai_constraint(args):
        set_up_script(args, "image_prompt")
    else:
        shutil.copy2(args.convert["out_json"], args.convert_report_path)
...
def build_parser():
    ...
    parser.add_argument(
        "--center",
        type=str,
        choices=["CENTER_1_RED_HOUSE", "CENTER_2_RENMING", "CENTER_4_WENZHOU", "CENTER_5_XIEHE"],
    ...
    parser.add_argument(
        "--use_openai_constraint",
        type=int,
        choices=[0, 1],
        default=None,
        help="Whether to optionally use an OpenAI-compatible API step to add image-based constraint text before final inference.",
    )
    parser.add_argument(
        "--use_image_prompt",
        type=int,
...
def should_use_openai_constraint(args) -> bool:
    if getattr(args, "use_openai_constraint", None) is not None:
        return bool(args.use_openai_constraint)
    return bool(getattr(args, "use_image_prompt", 0))
